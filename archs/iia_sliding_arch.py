...
class IIABWindowAttention(nn.Module):
    r""" IIAB Window based multi-head self attention (W-MSA) module with relative position bias.
    ...
    def forward(self, x, mask=None):
        ...
        feature = (attn @ v).transpose(1, 2)
        x = feature.reshape(B_, N//3, C)
...
class IIABSwinTransformerBlock(nn.Module):
    r""" IIAB Swin Transformer Block.
    ...
    def forward(self, x, cond, x_size, attn_mask):
        ...
        shifted_x = window_reverse_single(attn_windows, self.window_size, b, Hp, Wp)  # B D' H' W' C

        # reverse cyclic shift
        if any(i > 0 for i in self.shift_size):
            x = torch.roll(
                shifted_x, shifts=(self.shift_size[1], self.shift_size[2]), dims=(1, 2))
        ...
        if pad_d1 > 0 or pad_r > 0 or pad_b > 0:
            x = x[:, :h, :w, :].contiguous()
        # FFN
        x = shortcut + self.drop_path(x.contiguous())
    ...
    def forward(self, x, cond, x_size,attn_mask):
        ...
        for blk_idx, blk in enumerate(self.blocks):      
            if self.use_checkpoint:
                x= checkpoint.checkpoint(blk,x,cond, x_size,attn_mask)
            else:
                x = blk(x,cond, x_size,attn_mask)
...
class RSTB(nn.Module):
    ...
    def forward(self, x, cond, x_size,attn_mask):
        #n, c, h, w = x.shape
        x_ori = x
        x = self.residual_group(x, cond, x_size,attn_mask)
        x = self.conv(x)
        x = x.permute(0,2,3,1)
        x = x + x_ori
        return x
...
class SwinIRFM(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:    
            x = layer(x, cond, x_size , attn_mask)
